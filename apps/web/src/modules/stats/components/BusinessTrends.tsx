    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className={cn(
          ...
          <p className="text-xs text-muted-foreground">Перегруженные дни</p>
      ...
      <div>
          ...
          {routeLoad.map(({ day, stops, overloaded }) => {
            ...
            return (
              <div key={day} className="flex items-center gap-2">
                <span className="w-8 text-xs text-muted-foreground">{DAY_LABELS[day]}</span>
        ...
        {maxStopsPerDay > 0 && (
          <p className="mt-1 text-xs text-muted-foreground">Порог перегрузки: {maxStopsPerDay} точек/день</p>
        )}
      </div>
    </div>
  )
