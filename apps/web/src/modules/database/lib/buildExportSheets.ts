...
function stopTotalCost(clientMats: MatSpec[], priceMap: Map<string, number>): number {
  return clientMats.reduce((sum, m) => sum + (priceMap.get(m.size) ?? 0) * m.quantity, 0)
}
...
export function buildExportSheets({
  ...
  const priceMap = new Map(matSizes.map((s) => [s.id, s.rentalPrice]))
  ...
  const routeSheet: SheetData = {
    name: 'Маршруты',
    header: ['День', 'Позиция', 'Клиент', 'Водитель', 'Статус', 'Коврики', 'Площадь (м²)', 'Стоимость (₽)'],
    rows: routes.flatMap((route) =>
      route.stops.map((stop) => {
        ...
        return [
          ...
          Math.round(stopTotalCost(mats, priceMap) * 100) / 100,
        ]
      }),
    ),
  }
  ...
  summaryRows.push(['Размер', 'Количество', 'Площадь (м²)', 'Стоимость (₽)'])
  const matTotals = new Map<string, { qty: number; area: number; cost: number }>()
  for (const c of clients) {
    if (!c.isActive) continue
    for (const m of c.mats) {
      const prev = matTotals.get(m.size) ?? { qty: 0, area: 0, cost: 0 }
      const a = matArea(m.size, areaMap)
      const price = priceMap.get(m.size) ?? 0
      matTotals.set(m.size, {
        qty: prev.qty + m.quantity,
        area: prev.area + a * m.quantity,
        cost: prev.cost + price * m.quantity,
      })
    }
  }
  let totalQty = 0
  let totalArea = 0
  let totalRevenue = 0
  for (const [sizeId, { qty, area, cost }] of matTotals) {
    const label = matSizes.find((s) => s.id === sizeId)?.label ?? sizeId
    summaryRows.push([label, qty, Math.round(area * 100) / 100, Math.round(cost * 100) / 100])
    totalQty += qty
    totalArea += area
    totalRevenue += cost
  }
  summaryRows.push(['ИТОГО', totalQty, Math.round(totalArea * 100) / 100, Math.round(totalRevenue * 100) / 100])

  // 6b. Нагрузка по дням
  summaryRows.push(['', '', '', ''])
  summaryRows.push(['— Нагрузка по дням —', '', '', ''])
  summaryRows.push(['День', 'Остановок', 'Площадь (м²)', 'Стоимость (₽)'])
  for (const route of routes) {
    ...
    for (const stop of route.stops) {
      ...
      dayCost += stopTotalCost(mats, priceMap)
    }
    summaryRows.push([
      DAY_LABELS_FULL[route.day as DayOfWeek],
      route.stops.length,
  ...
  summaryRows.push(['Водитель', 'Остановок', 'Площадь (м²)', 'Стоимость (₽)'])
  const driverLoad = new Map<string, { stops: number; area: number; cost: number }>()
  for (const route of routes) {
    for (const stop of route.stops) {
      const driverName = stop.driverId
        ? (driverMap.get(stop.driverId) ?? stop.driverId)
        : 'Без водителя'
      const prev = driverLoad.get(driverName) ?? { stops: 0, area: 0, cost: 0 }
      const mats = clientMap.get(stop.clientId)?.mats ?? []
      driverLoad.set(driverName, {
        stops: prev.stops + 1,
        area: prev.area + stopTotalArea(mats, areaMap),
        cost: prev.cost + stopTotalCost(mats, priceMap),
      })
    }
  }
  for (const [name, { stops, area, cost }] of driverLoad) {
    summaryRows.push([name, stops, Math.round(area * 100) / 100, Math.round(cost * 100) / 100])
  }
